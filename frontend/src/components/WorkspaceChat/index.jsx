...
import { LocalWhisper } from "@/utils/WhisperProviders/localWhisper";
...
// Function to generate speech using LocalWhisper
async function generateSpeech(text, voiceSettings) {
  const tts = new LocalWhisper({ options: { WhisperModelPref: "Xenova/whisper-small" } });
  const audioBuffer = await tts.ttsBuffer(text, voiceSettings);
  // Handle the audio buffer (e.g., save to file, stream to client, etc.)
}
...
export default function WorkspaceChat({ loading, workspace }) {
  ...
  const voiceSettings = workspace?.avatarSettings;

  const handleTextToSpeech = async (text) => {
    await generateSpeech(text, voiceSettings);
  };
  ...
  return (
    <div>
      ...
      <button onClick={() => handleTextToSpeech("Hello, world!")}>
        Speak
      </button>
    </div>
  );
